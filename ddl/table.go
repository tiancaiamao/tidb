...
func onEnableCachedTable(t *meta.Meta, job *model.Job) (ver int64, err error) {
	tbInfo, err := getTableInfoAndCancelFaultJob(t, job, job.SchemaID)
	if err != nil {
		return 0, errors.Trace(err)
	}
	if tbInfo.CachedTableStatusType == model.CachedTableENABLE {
		job.State = model.JobStateCancelled
		return ver, errors.Trace(errors.Errorf("Table (pid : %d) is been Cached , So CACHED ON its a invalid operator", job.TableID))
	}

	switch tbInfo.CachedTableStatusType {
	case model.CachedTableDISABLE:
		// disable -> switching
		tbInfo.CachedTableStatusType = model.CachedTableSWITCHING
	...
	case model.CachedTableSWITCHING:
		// switching -> enable
		tbInfo.CachedTableStatusType = model.CachedTableENABLE
		ver, err = updateVersionAndTableInfoWithCheck(t, job, tbInfo, true)
		if err != nil {
			return ver, err
		}
	...
	default:
		job.State = model.JobStateCancelled
		err = ErrInvalidDDLState.GenWithStackByArgs("table cached", tbInfo.CachedTableStatusType.String())
	}
	return ver, err
}
func onDisableCachedTable(t *meta.Meta, job *model.Job) (ver int64, err error) {

	tbInfo, err := getTableInfoAndCancelFaultJob(t, job, job.SchemaID)
	if err != nil {
		return 0, errors.Trace(err)
	}
	if tbInfo.CachedTableStatusType == model.CachedTableDISABLE {
		job.State = model.JobStateCancelled
		return ver, errors.Trace(errors.Errorf("Table (pid : %d) is not been cached , So CACHED OFF its a invalid operator", job.TableID))
	}
	switch tbInfo.CachedTableStatusType {
	case model.CachedTableENABLE:
		//  enable ->  switching
		tbInfo.CachedTableStatusType = model.CachedTableSWITCHING
		ver, err = updateVersionAndTableInfoWithCheck(t, job, tbInfo, true)
		if err != nil {
	...
	case model.CachedTableSWITCHING:
		// switching -> disable
		tbInfo.CachedTableStatusType = model.CachedTableDISABLE
		ver, err = updateVersionAndTableInfoWithCheck(t, job, tbInfo, true)
		if err != nil {
			return ver, err
		}
